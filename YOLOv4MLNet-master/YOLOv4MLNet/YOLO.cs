...
namespace YOLOv4MLNet
{
    ...
    {
        string modelPath;
        static public readonly string[] classesNames = new string[] { "person","bicycle", "car", "motorbike", "aeroplane", "bus", "train",
            ...
            "hot dog", "pizza", "donut", "cake", "chair", "sofa", "pottedplant","bed", "diningtable", "toilet", "tvmonitor", "laptop", "mouse",
        ...
        public async Task<bool> StartYOLO(Model viewmodel, string pathToImage, CancellationToken ct)
        {
            return await Task<bool>.Factory.StartNew(() =>
            {
                using (var bitmap = new Bitmap(Image.FromFile(pathToImage)))
                {
                    ...
                    Images imgDesc = new Images(pathToImage);
                    foreach (var res in results)
                    {
                        ...
                        var x2 = res.BBox[2];
                        var y2 = res.BBox[3];
                        Objects objDesc = new Objects(x1, y1, y2 - y1, x2 - x1, res.Label);
                        imgDesc.Add(objDesc);
                    }
